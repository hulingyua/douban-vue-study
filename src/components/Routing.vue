<template>
    <div class="routing" v-if='rating'>
        <template v-if="full == 0">
            暂无评分
        </template>
        <template v-else>
            <template v-for="(f, fI) in full">
                <span class="star-full"></span>
            </template>
            <template v-for="(f, fI) in half">
                <span class="star-half"></span>
            </template>
            <template v-for="(f, fI) in gray">
...
export default {
    ...
    created() {
        ...
        let full = parseInt(average / 2);
        let half = full % 2 === 0 ? 0 : 1;
